...
diags_dir='c:/Users/Gianni/Desktop/PIC/1DPIC/Data'
...
for v, color in zip(all_species, colors):
    axs.scatter(v.px,v.x, alpha=0.8, c=color, edgecolors='none', s=30, label=v.name)
...
#    for v, color in zip(all_species, colors):
#        plt.scatter(n,v.x, alpha=0.8, c=color, edgecolors='none', s=30, label=v.name)
...
# initialize the EM field to a laser pulse
a0=1
omega=1
k=omega
fwhm =15*fs
...
Jy= np.zeros((nx))
Jz= np.zeros((nx))
U_Ex_tot = np.zeros((nt))
U_Ey_tot = np.zeros((nt))
U_Ez_tot = np.zeros((nt))
U_Bx_tot = np.zeros((nt))
U_By_tot = np.zeros((nt))
U_Bz_tot = np.zeros((nt))
U_em_tot = np.zeros((nt))

fig = plt.figure(figsize=(12, 8))
ax = fig.gca(xlabel="x [code units]", ylabel="E field [code units]", title='Ey')
max_mus=int(np.floor(nt/out_freq))
mus=np.array([i+1 for i in range(max_mus)])
colorparams = mus
colormap = cm.viridis
...
for i in range(nt):
    Bx, By, Bz = advance_B(dx, dt, Ex, Ey, Ez, Bx, By, Bz)
    Bx, By, Bz = pbc_B(Bx, By, Bz)
    Ex, Ey, Ez = advance_E(dx, dt, Ex, Ey, Ez, Bx, By, Bz, Jx, Jy, Jz)
    Ex, Ey, Ez = pbc_E(Ex, Ey, Ez)
    U_Ex_tot[i], U_Ey_tot[i], U_Ez_tot[i], U_Bx_tot[i],U_By_tot[i],U_Bz_tot[i], U_em_tot[i] = compute_emfield_energy(x, Ex, Ey, Ez, Bx, By, Bz)
    if (i % out_freq == 0):
        print(i)
        color = colormap(normalize((i+out_freq-1)/out_freq))
        ax.plot(x,Bz,color=color,lw=3)
#        save_1Dquantity(x, np.c_[Ex,Ey,Ez,Bx,By,Bz], 'EM_FIELD_%04d.txt' % i)
fig = plt.figure(figsize=(12, 8))
ax = fig.gca(xlabel="t [code units]", ylabel="field energy [code units]", title='energy contributions')
ax.plot(t,U_Ex_tot,label='Ex')
ax.plot(t,U_Ey_tot,label='Ey')
ax.plot(t,U_Ez_tot,label='Ez')
ax.plot(t,U_Bx_tot,label='Bx')
ax.plot(t,U_By_tot,label='By')
ax.plot(t,U_Bz_tot,label='Bz')
ax.plot(t, U_em_tot,label='E$_{em}$')
ax.legend()